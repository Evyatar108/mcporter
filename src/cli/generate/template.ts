...
export function renderTemplate({
  ...
  const embedded = JSON.stringify(defWithLifecycle, (_key, value) => (value instanceof URL ? value.toString() : value), 2);
  ...
  const renderedTools = toolDocs.map((entry) => ({
    ...renderToolCommand(entry.tool, timeoutMs, serverName, entry.doc, defaults),
    doc: entry.doc,
    tool: entry.tool,
  }));
